...
/**
 * Settings Page
 * Phase 4: Complete settings page implementation
...
import { successMessages, errorMessages } from '@/config/messages';
...
export default function SettingsPage() {
  ...
  // Phase 3 Fix: Removed settings.language field (now managed via user.language_preference)
  ...
  // Phase 3 Fix: Removed settings.language initialization (now managed via user.language_preference)
  useEffect(() => {
    if (settings) {
      setFormValues({
        ...
        preferences: {
          ...
          defaultLanguage: settings.preferences?.defaultLanguage || 'en',
  ...
  // Phase 3 Fix: Removed handleLanguageChange (language now managed via LanguageContext)
  ...
  const handleSave = async () => {
    ...
    if (success) {
      toast.success(t('common:messages.settingsSaved', 'Settings saved successfully'));
  ...
  // Phase 3 Fix: Removed settings.language comparison (now managed via user.language_preference)
    ...
    <div className={cn("max-w-4xl mx-auto", spacing.vertical.lg, spacing.container.pagePadding)}>
      ...
      <div className={cn("space-y-6", spacing.vertical.lg)}>
        ...
        >
          ...
          />
          {/* Phase 3 Fix: Added LanguageSelector component for UI language
              Uses user.language_preference (single source of truth)
        ...
        >
          <SelectDropdown
            label={t('settings:preferences.defaultLanguage.label')}
            description={t('settings:preferences.defaultLanguage.description')}
            value={formValues.preferences?.defaultLanguage || 'en'}
            onValueChange={(value) => handlePreferenceToggle('defaultLanguage', value)}
